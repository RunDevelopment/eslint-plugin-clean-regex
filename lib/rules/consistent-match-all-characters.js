		...
		return createRuleListener(({ visitAST, pattern, flags, replaceElement, replaceLiteral }) => {
			...
			visitAST({
				onCharacterClassEnter(node) {
					if (node.raw !== replacement && util.isMatchAll(node, flags)) {
						nodesToReplace.push({
							node,
			...
			if ((replacement === ".") === flags.dotAll) {
				...
				context.report({
					message: `${nodesToReplace.length} element(s) in the pattern will be replaced with ${mention(replacement)} and the s flag will be ${replacement === "." ? "added" : "removed"}.`,
