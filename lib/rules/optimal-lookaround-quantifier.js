				...
				onAssertionEnter(node) {
					if (node.kind === "lookahead" || node.kind === "lookbehind") {
						...
						for (const q of quantifiers) {
							...
							context.report({
								message: `The quantified expression ${q.raw} at the ${endOrStart} of the expression tree should only be matched a constant number of times. The expression can be ${proposal} without affecting the lookaround.`,
								...reportElement(q)
							});
						}
					}
