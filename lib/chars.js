...
function getUnicodeCaseVariations() {
	...
	for (let i = 0; i <= 0x10FFFF; i++) {
		...
		// Upper and lower case is strange in Unicode
		if (upper.toLowerCase() !== lower || lower.toUpperCase() !== upper) {
			continue;
		}
 ...
 *
 * @param {Iterable<number | CharRange | Readonly<CharacterSet | CharacterClassElement>>} chars The characters in the set.
		...
		if (typeof char == "number") {
			...
			switch (char.type) {
				...
				case "CharacterClassRange":
					addRange(char.min.value, char.max.value);
					break;
				...
				default:
					throw assertNever(char, "Invalid element type");
			}
		} else {
...
module.exports = {
	Chars,
	toChars,
	DIGIT, SPACE, WORD, LINE_TERMINATOR
};
