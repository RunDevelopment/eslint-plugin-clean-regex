			...
			function process(node) {
				...
				if (alts.every(a => a.isCharacter)) {
					...
					if (alts.length >= 3 || findNonDisjointAlt(alts) || totalIsAll(alts)) {
						const [prefix, suffix] = node.type === "Group" ? ["", ""] : util.getParentPrefixAndSuffix(node);
				...
				if (nonDisjointAlt || characterAlternativesCount >= 3 || totalIsAll(alts)) {
					...
					const [prefix, suffix] = util.getParentPrefixAndSuffix(node);
					...
					if (newRaw !== node.raw) {
						...
						const firstChanged = util.findIndex(genericAlts, (a, i) => a.raw !== node.alternatives[i].raw);
						const lastChanged = util.findLastIndex(
							genericAlts,
