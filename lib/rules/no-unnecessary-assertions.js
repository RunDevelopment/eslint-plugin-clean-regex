		...
		return createRuleListener(({ visitAST, flags, reportElement }) => {
				...
				if (nextChar.nonEmpty) {
					...
					if (!flags.multiline) {
						...
						if (nextChar.char.isSubsetOf(dot)) {
							context.report({
								message: `${mention(node)} will always reject because it is ${followed} by a non-line-terminator character.`,
								...reportElement(node)
							});
						} else if (nextChar.char.isDisjointWith(dot)) {
							context.report({
								message: `${mention(node)} will always accept because it is ${followed} by a line-terminator character.`,
								...reportElement(node)
							});
						}
					}
			...
			function checkWordBoundary(node) {
				...
				if (prevIsNonWord) {
					...
					if (nextIsWord) {
						context.report({
							message: `${mention(node)} will always ${accept} because it is preceded by a non-word character and followed by a word character.`,
							...reportElement(node)
						});
					}
					if (nextIsNonWord) {
						context.report({
							message: `${mention(node)} will always ${reject} because it is preceded by a non-word character and followed by a non-word character.`,
							...reportElement(node)
						});
					}
				}
				if (prevIsWord) {
					...
					if (nextIsNonWord) {
						context.report({
							message: `${mention(node)} will always ${accept} because it is preceded by a word character and followed by a non-word character.`,
							...reportElement(node)
						});
					}
					if (nextIsWord) {
						context.report({
							message: `${mention(node)} will always ${reject} because it is preceded by a word character and followed by a word character.`,
							...reportElement(node)
						});
					}
				}
