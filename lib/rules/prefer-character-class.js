...
/**
 * Returns the string representation of the given character class elements in a non-negated character class.
 *
 * @param {Readonly<Simple<CharacterClassElement>>[]} elements
 * @returns {string}
 */
function characterClassElementToCharacterClass(elements) {
		...
		return createRuleListener(({ visitAST, replaceElement }) => {
			...
			function process(node) {
				...
				function combineRun(runIndex, runLength) {
					...
					return alternatives.slice(runIndex, runIndex + runLength).some(alt => alt.elements[0].type === "CharacterClass");
				}
				...
				if (elements.every(Boolean)) {
					...
					const replacement = prefix + characterClassElementToCharacterClass(charElements) + suffix;
					...
					for (let i = 0; i < elements.length; i++) {
						...
						foundRuns = true;
						newAlternatives.push(characterClassElementToCharacterClass(runElements));
						i += runLength - 1;
					}
