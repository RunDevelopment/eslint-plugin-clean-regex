...
/**
 *
 * @param {readonly (CharacterClassElement | CharacterSet)[]} elements
 * @param {Readonly<Flags>} flags
 * @returns {Chars}
 */
function elementToChars(elements, flags) {
	if (flags.dotAll && elements.some(e => e.type === "CharacterSet" && e.kind === "any")) {
		return Chars.all(flags.unicode ? 0x10FFFF : 0xFFFF);
	}
	return toChars(elements.map(e => {
		switch (e.type) {
			case "Character":
				return e.value;
...
function doMatchAll(elements, flags) {
	return elementToChars(elements, flags).isAll;
}
