			...
			visitAST({
				onAssertionEnter(node) {
					...
					if (q && isOptional(node, q)) {
						context.report({
							message: `This assertion effectively optional and does not change the pattern. Either remove the assertion or change the parent quantifier (${q.raw.substr(q.element.raw.length)}).`,
							...reportElement(node)
						});
					}
				}
