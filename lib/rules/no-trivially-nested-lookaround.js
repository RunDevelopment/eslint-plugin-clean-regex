/**
 * @fileoverview Rule to disallow trivially nested lookarounds
				...
				visitRegExpAST(ast, {
					onAssertionEnter(node) {
						if (node.kind === "lookahead" || node.kind === "lookbehind") {
							...
							if (replacement) {
								context.report({
									message: `Trivially nested lookaround: The outer ${node.kind} is unnecessary.`,
									...reportElement(regexLiteral, node, replacement)
								});
							}
						}
					}
