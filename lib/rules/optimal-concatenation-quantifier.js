...
const { toChars, Chars } = require("../chars");
 ...
 * @param {Flags} flags
 * @returns {{ chars: Chars; complete: boolean }}
 */
function createChars(element, flags) {
	function empty() {
		return {
			chars: Chars.empty(flags.unicode ? 0x10FFFF : 0xFFFF),
			complete: false
		};
	}
	...
	switch (element.type) {
		...
		case "CharacterSet":
			return {
				chars: toChars([element], flags),
				complete: true
			};

		case "CharacterClass": {
			const chars = toChars(element.elements, flags);
			return {
				chars: element.negate ? chars.negate() : chars,
				complete: true
			};
		}
				...
				onAlternativeEnter(node) {
					for (let i = 0; i < node.elements.length - 1; i++) {
						...
						if (current.type === "Quantifier" && current.min !== current.max
							...
							if (next.max === Infinity && currChars.complete && nextChars.chars.hasEveryOf(currChars.chars)) {
							...
							else if (current.max === Infinity && nextChars.complete && currChars.chars.hasEveryOf(nextChars.chars)) {
							...
							if (replacement) {
								...
								context.report({
									message: `The quantifiers of ${current.raw}${next.raw} are not optimal and should be replaced with ${replacement}.`,
