...
testRule(__filename, undefined, {
	...
	invalid: [
		{ code: String(/[\wa]/), output: String(/[\w]/), errors: [{ message: "`a` (\\x61) is already included by `\\w` ([0-9A-Za-z_])." }] },
		{ code: String(/[a\w]/), output: String(/[\w]/), errors: [{ message: "`a` (\\x61) is already included by `\\w` ([0-9A-Za-z_])." }] },
		...
		{ code: String(/[\wa-f]/), output: String(/[\w]/), errors: [{ message: "`a-f` (\\x61-\\x66) is already included by `\\w` ([0-9A-Za-z_])." }] },
		{ code: String(/[a-fA-F]/i), output: String(/[a-f]/i), errors: [{ message: "`A-F` (\\x41-\\x46) is already included by `a-f` (\\x61-\\x66)." }] },
		{ code: String(/[0-9\w]/i), output: String(/[\w]/i), errors: [{ message: "`0-9` (\\x30-\\x39) is already included by `\\w` ([0-9A-Za-z_])." }] },
		{ code: String(/[a-f\D]/i), output: String(/[\D]/i), errors: [{ message: "`a-f` (\\x61-\\x66) is already included by `\\D` ([^0-9])." }] },

		{ code: String(/[\w\d]/i), output: String(/[\w]/i), errors: [{ message: "`\\d` ([0-9]) is already included by `\\w` ([0-9A-Za-z_])." }] },
		...
		{ code: String(/[\S\d]/), output: String(/[\S]/), errors: [{ message: "`\\d` ([0-9]) is already included by `\\S`." }] },
		...
		{ code: String(/[\w\p{ASCII}]/u), output: String(/[\p{ASCII}]/u), errors: [{ message: "`\\w` ([0-9A-Za-z_]) is already included by `\\p{ASCII}`." }] },
	]
});
