...
module.exports = {
	...
	create(context) {
		return createRuleListener(({ pattern, parseExpression, replaceElement }) => {
			...
			visitRegExpAST(pattern, {
				onGroupEnter(node) {
					...
					// the case /(?:)/ is equivalent to an empty regex and should be legal
					if (elements.length === 0 && parent.type === "Alternative" && parent.parent.type === "Pattern" &&
						parent.parent.raw === "(?:)") {
						return;
					}
