...
const util = {
	...
	isEscapeSequence(node) {
		return util.isOctalEscapeSequence(node) // octal
	...
	toCharSet(elements, flags) {
		if (Array.isArray(elements)) {
			return JS.createCharSet(elements.map(e => {
				switch (e.type) {
					case "Character":
						return e.value;
					case "CharacterClassRange":
	...
	/**
	 * Returns the number of unique character in the given character set.
	 *
	 * @param {CharSet} set
	 * @returns {number}
	 */
	 ...
	 * @property {boolean} nonEmpty If `false`, then the first character also includes the empty word.
		...
		switch (element.type) {
			...
			case "Backreference": {
				...
				if (exact) {
					...
					exact = util.charSetSize(resolvedChar.char) === 1;
				}
	...
	/**
	 * Returns the first character after the given assertion.
	 *
	 * What "after" means depends the on the given direction which will be interpreted as the current matching
	 * direction. You can use this to get the previous character of an assertion as well.
	 *
	 ...
	 * @returns {FirstChar}
	 */
	getFirstCharAfterAssertion(assertion, direction, flags) {
		...
		let nextElements = advanceElement(assertion, direction);
