		...
		return createRuleListener(({ visitAST, flags, removeElement, replaceElement }) => {
			...
			function checkRangeElements(range) {
				if (range.min.value === range.max.value) {
					let raw = range.min.raw === "-" ? "\\-" : range.min.raw;
					context.report({
						message: `${range.raw} includes only a single value (${hex(range.min.value)}).`,
						...replaceElement(range, raw)
					});
				}

				if (range.min.value + 1 === range.max.value) {
					let min = range.min.raw === "-" ? "\\-" : range.min.raw;
					let max = range.max.raw === "-" ? "\\-" : range.max.raw;
					context.report({
						message: `${range.raw} includes only its two ends (${hex(range.min.value)} and ${
							hex(range.max.value)}).`,
						...replaceElement(range, min + max)
					});
				}
			}
			...
			visitAST({
				onCharacterClassEnter(node) {
					...
					if (elements.length <= 1) {
						// char classes with <= 1 elements are always optimal (almost)
						const first = elements[0];
						if (first.type === "CharacterClassRange") {
							checkRangeElements(first);
						}
						return;
					}
