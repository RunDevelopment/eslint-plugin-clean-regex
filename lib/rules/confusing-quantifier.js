			...
			visitAST({
				onQuantifierEnter(node) {
					if (node.min > 0 && isPotentiallyEmpty(node.element)) {
						...
						context.report({
							message: `This quantifier is confusing because its minimum is ${node.min} but it can match the empty string. Maybe replace it with ${mention(proposal)} to reflect that it can match the empty string?`,
