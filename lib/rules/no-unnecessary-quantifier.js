			...
			visitAST({
				onQuantifierEnter(node) {
					...
					if (node.min === 0 && node.max === 1 && util.isPotentiallyEmpty(node.element)) {
						context.report({
							message: "The optional quantifier can be removed because the quantified element can match the empty string.",
							...reportQuantifier(node)
						});
						return;
					}
					...
					if (node.min > 0 && util.isZeroLength(node.element)) {
						context.report({
							message: "The quantified element does not consume characters, so the quantifier (minimum > 0) can be removed.",
							...reportQuantifier(node)
						});
						return;
					}
