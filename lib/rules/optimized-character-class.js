"use strict";

const { createRuleListener, getDocUrl } = require("../rules-util");
const util = require("../util");

/**
 * @typedef {import("regexpp/ast").Character} Character
 * @typedef {import("regexpp/ast").CharacterClassElement} CharacterClassElement
 * @typedef {import("regexpp/ast").CharacterClassRange} CharacterClassRange
 * @typedef {import("regexpp/ast").CharacterSet} CharacterSet
 */

/**
 *
 * @param {CharacterClassElement} element
 * @returns {string}
 */
function elementToString(element) {
	switch (element.type) {
		case "Character":
			return `${element.raw} (${util.minimalHexEscape(element.value)})`;
		case "CharacterClassRange":
			return `${element.raw} (${util.minimalHexEscape(element.min.value)}-${util.minimalHexEscape(element.max.value)})`;
		case "CharacterSet":
			return element.raw;
		default:
			throw util.assertNever(element);
	}
}


/** @type {import("eslint").Rule.RuleModule} */
module.exports = {
	meta: {
		type: "suggestion",
		docs: {
			description: "Disallows unnecessary elements in character classes.",
			category: "Best Practices",
			url: getDocUrl(__filename)
		},
		fixable: "code"
	},

	create(context) {
		return createRuleListener(({ visitAST, flags, removeElement, replaceElement }) => {

			visitAST({
				onCharacterClassEnter(node) {
					const elements = node.elements;
					if (elements.length === 0) {
						// it's optimal as is
						return;
					}

					/**
					 * @typedef Report
					 * @property {string} message
					 * @property {string} replacement
					 *
					 * @typedef {import("regexpp/ast").CharacterClassElement} CharacterClassElement
					 *
					 * @type {Map<CharacterClassElement, Report>}
					 */
					const reports = new Map();

					/**
					 * @param {CharacterClassRange} range
					 * @returns {void}
					 */
					function checkRangeElements(range) {
						// Note: These reports may later be overwritten should the range be removed.

						if (range.min.value === range.max.value) {
							reports.set(range, {
								message: `${elementToString(range)} contains only a single value.`,
								replacement: range.min.raw === "-" ? "\\-" : range.min.raw
							});
						}

						if (range.min.value + 1 === range.max.value) {
							const min = range.min.raw === "-" ? "\\-" : range.min.raw;
							const max = range.max.raw === "-" ? "\\-" : range.max.raw;
							reports.set(range, {
								message: `${elementToString(range)} contains only its two ends.`,
								replacement: min + max
							});
						}
					}

					for (const element of elements) {
						if (element.type === "CharacterClassRange") {
							checkRangeElements(element);
						}
					}


					// detect duplicates and subsets

					// This will be done in 3 phases. First we check all single characters, then all characters ranges,
					// and last all character sets. This will ensure that we keep character sets over character ranges,
					// and character ranges over character sets.

					const empty = util.toCharSet([], flags);
					const elementChars = elements.map(e => util.toCharSet([e], flags));

					/** @type {CharacterClassElement["type"][]} */
					const order = ["Character", "CharacterClassRange", "CharacterSet"];
					for (const currentType of order) {
						for (let i = elements.length - 1; i >= 0; i--) {
							const current = elements[i];
							if (current.type !== currentType) {
								continue;
							}

							const currentChars = elementChars[i];
							const totalWithCurrent = empty.union(...elementChars.filter((_, index) => index !== i));

							if (totalWithCurrent.hasEveryOf(currentChars)) {
								elementChars[i] = empty;

								// try to find a single element which is still a superset
								/** @type {CharacterClassElement | undefined} */
								let simpleSuper = undefined;
								for (let k = 0; k < elements.length; k++) {
									if (elementChars[k].hasEveryOf(currentChars)) {
										simpleSuper = elements[k];
										break;
									}
								}

								if (simpleSuper === undefined) {
									reports.set(current, {
										message: `${elementToString(current)} is already included by some combination of other elements.`,
										replacement: ""
									});
								} else {
									reports.set(current, {
										message: `${elementToString(current)} is already included by ${elementToString(simpleSuper)}.`,
										replacement: ""
									});
								}
							}
						}
					}


					for (const [element, { message, replacement }] of reports) {
						if (replaceElement) {
							context.report({
								message,
								...replaceElement(element, replacement)
							});
						} else {
							context.report({
								message,
								...removeElement(element)
							});
						}
					}
				}
			});

		});
	}
};
