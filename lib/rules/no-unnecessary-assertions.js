	...
	create(context) {
		...
		return createRuleListener(({ visitAST, flags, reportElement }) => {
			...
			function checkStartOrEnd(node) {
				...
				const nextChar = util.getFirstCharAfterAssertion(node, direction, flags);
			...
			function checkWordBoundary(node) {
				...
				const next = util.getFirstCharAfterAssertion(node, "ltr", flags);
				const prev = util.getFirstCharAfterAssertion(node, "rtl", flags);
			...
			function checkLookaround(node) {
				...
				const after = util.getFirstCharAfterAssertion(node, direction, flags);
				if (!after.nonEmpty) {
					return;
				}
