...
const util = {
	 ...
	 *
	 * @param {Element | Alternative | Alternative[]} element
	 * @param {Readonly<IsPotentiallyEmptyOptions>} [options]
	 * @returns {boolean}
	 *
	 * @typedef IsPotentiallyEmptyOptions
	 * @property {boolean} [backreferencesAreNonEmpty] If `true`, then backreferences that aren't guaranteed to always
	 * be replaced with the empty string will be assumed to be non-empty.
	 */
	isPotentiallyEmpty(element, options = {}) {
		if (Array.isArray(element)) {
			return element.some(a => util.isPotentiallyEmpty(a, options));
		}

		switch (element.type) {
			case "Alternative":
				return element.elements.every(e => util.isPotentiallyEmpty(e, options));

			case "Assertion":
				// assertion do not consume characters but they do usually reject some pre- or suffixes
				if (element.kind === "lookahead" || element.kind === "lookbehind") {
					if (!element.negate && util.isPotentiallyEmpty(element.alternatives, options)) {
						// if a positive lookaround is potentially empty, it will trivially accept all pre- or suffixes
						return true;
					}
				}
			...
			case "Backreference":
				...
				if (options.backreferencesAreNonEmpty) {
					...
					return (
						util.backreferenceAlwaysAfterGroup(element) ||
						util.isPotentiallyEmpty(element.resolved, options)
					);
				}
			...
			case "CapturingGroup":
			case "Group":
				return util.isPotentiallyEmpty(element.alternatives, options);

			case "Quantifier":
				return element.min === 0 || util.isPotentiallyEmpty(element.element, options);
	 ...
	 */
	getParentPrefixAndSuffix(parent) {
		switch (parent.type) {
			case "Assertion":
