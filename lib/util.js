...
const util = {
	...
	/**
	 * Returns whether the given element might not move the position of the automation.
	 *
	 * @param {Element} element
	 * @returns {boolean}
	 */
	isPotentiallyZeroLength(element) {
		switch (element.type) {
			...
			case "Quantifier":
				return element.min === 0 || util.isPotentiallyZeroLength(element);
	 ...
	 *
	 * The descendants will be iterated in a DFS manner from left to right with the first node being the given node.
	 *
	 * @param {Node & T} node
	 * @param {(descendant: import("./util-types").Descendants<T>) => boolean} conditionFn
	 * @param {(descendant: import("./util-types").Descendants<T>) => boolean} [descentConditionFn]
	 * @template T
	 */
