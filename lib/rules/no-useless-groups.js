/**
 * @fileoverview Rule to disallow useless non-capturing groups
...
module.exports = {
	meta: {
		...
		docs: {
			description: "disallow useless non-capturing groups",
				...
				onGroupEnter(node) {
					...
					if (node.alternatives.length === 1) {
						...
						if (concatenation.length === 0) {
							context.report({
								message: "Useless group: Empty non-capturing group.",
								...reportElement(node)
							});
							return;
						}
						...
						if (concatenation.length === 1) {
							...
							if (type === "CapturingGroup" || type === "Character" || type === "CharacterClass" ||
								...
								context.report({
									message: `Useless group: The non-capturing groups around ${element.raw} is unnecessary.`,
									...reportElement(node)
								});
								return;
							}
						...
						if (node.parent.type !== "Quantifier") {
							context.report({
								message: `Useless group: The non-capturing group ${node.raw} has neither quantifiers nor does it contain alternations and this therefore unnecessary.`,
								...reportElement(node)
							});
							return;
						}
