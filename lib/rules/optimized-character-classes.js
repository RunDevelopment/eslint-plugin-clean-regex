/**
 * @fileoverview Rule to Optimized character classes
 * @author Michael Schmidt
 */
"use strict";

const { RegExpParser, visitRegExpAST } = require("regexpp");
const { createRuleListener, reportElement } = require("../rules-util");
...
module.exports = {
	meta: {
		...
		docs: {
			description: "Optimized character classes",
	...
	create(context) {
		...
		return createRuleListener({
			RegexLiteral(regexLiteral) {
				...
				const testFlags = flags.replace(/[mysg]/g, "");
					...
					onCharacterClassEnter(node) {
						...
						function checkChar(needle, index) {
							for (let i = 0; i < elements.length; i++) {
								...
								if (report) {
									context.report({
										message: `Optimized character class: ${needle.raw} (${
											hex(needle.value)}) is already included by ${element.raw}.`,
										...reportElement(regexLiteral, needle, "")
									});
									return true;
								}
							}
						...
						function checkSet(needle, index) {
							for (let i = 0; i < elements.length; i++) {
								...
								if (report) {
									context.report({
										message: `Optimized character class: ${needle.raw} is already included by ${element.raw}.`,
										...reportElement(regexLiteral, needle, "")
									});
									return true;
								}
